# These are files and external links to the tools needed to build
# the baserom, plus lists of files that are deleted since they
# are not relevant to the build process.

# Asar
$Asar_Dir = "$ToolsDir\asar\"
$Asar_Download = "https://dl.smwcentral.net/37443/"
$Asar_Junk = @("LICENSE", "license-lgpl.txt", "license-gpl.txt", "license-wtfpl.txt", "dll", "ext")
$Asar_Docs = @("docs", "README.txt")

# AddmusicK
$AddMusicK_Dir = "$ToolsDir\addmusick\"
$AddMusicK_Download = "https://dl.smwcentral.net/37906/"
$AddMusicK_Junk = @("src.zip", "addmusicMRemover.pl", "Makefile", "asar.exe")
$AddMusicK_Docs = @("readme_files", "readme.html")

# Flips
$Flips_Dir = "$ToolsDir\flips\"
$Flips_Download = "https://dl.smwcentral.net/11474/"
$Flips_Junk = @("license.txt", "flips-linux", "boring.zip", "src.zip")
$Flips_Docs = @("")

# GPS
$GPS_Dir = "$ToolsDir\gps\"
$GPS_Download = "https://dl.smwcentral.net/31515/"
$GPS_Junk = @("src.zip", "Changes.txt")
$GPS_Docs = @("README.txt")

# Lunar Magic
$LunarMagic_Dir = "$ToolsDir\lunar_magic\"
$LunarMagic_Download = "https://dl.smwcentral.net/38972/"
$LunarMagic_Junk = "readme.txt"
$LunarMagic_Docs = @("")

# PIXI
$PIXI_Dir = "$ToolsDir\pixi\"
$PIXI_Download = "https://dl.smwcentral.net/37432/"
$PIXI_Junk = @("removedResources.txt", "changelog.txt", "README.html", "CONTRIBUTING.html", "CHANGELOG.html", "LICENSE")
$PIXI_Docs = @("README.html")

# UberASM Tool
$UberASMTool_Dir = "$ToolsDir\uberasmtool\"
$UberASMTool_Download = "https://dl.smwcentral.net/36986/"
$UberASMTool_Junk = @("readme.txt", "changelog.txt", "incompatibilities.txt", "UberASMTool.dll.config")
$UberASMTool_Docs = @("readme.html")
